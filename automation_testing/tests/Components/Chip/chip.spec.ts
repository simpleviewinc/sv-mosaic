...
test.describe("Components - Chip - Kitchen Sink", () => {
	...
	test("Validate Chip has simplyGold background.", async () => {
		...
		expect(await chipPage.getBackgroundColorFromElement(chipPage.chip.nth(1))).toBe(expectedColor);
		expect(await chipPage.getBackgroundColorFromElement(chipPage.chip.nth(3))).toBe(expectedDisabledBgColor);
	...
	test("Validate Chip has almostBlack color as text.", async () => {
		...
		for (let i = 0; i < await chipPage.chip.count(); i++) {
			expect(await chipPage.getColorFromElement(chipPage.chip.nth(i))).toBe(expectedColor);
		}
	...
	test("Validate Chip has grey2 background.", async () => {
		...
		expect(await chipPage.getBackgroundColorFromElement(chipPage.chip.nth(0))).toBe(expectedColor);
		expect(await chipPage.getBackgroundColorFromElement(chipPage.chip.nth(2))).toBe(expectedColor);
	});
});
