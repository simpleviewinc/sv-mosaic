...
test.describe("Data View", () => {
	...
	test("Columns - Validate Default Columns Information", async () => {
	...
	test("Columns - Change column order - Down", async () => {
		await columns.columnsBtn.click();
		let downArrow = await columns.getDownArrowByIemName(dataview.changeItemDown);
		...
		downArrow = await columns.getDownArrowByIemName(dataview.changeItemDown);
		await downArrow.click();
		expect((await columns.getRightItemsText()).toString()).toBe(dataview.changeColumnsOrderDown.toString());
	});

	test("Columns - Change column order - Up", async () => {
		await columns.columnsBtn.click();
		let upArrow = await columns.getUpArrowByIemName(dataview.changeItemUp);
		...
		await upArrow.click();
		upArrow = await columns.getUpArrowByIemName(dataview.changeItemUp);
		await upArrow.click();
		expect((await columns.getRightItemsText()).toString()).toBe(dataview.changeColumnsOrderUp.toString());
	});

	test("Columns - No Chage column order - Down", async () => {
		await columns.columnsBtn.click();
	...
	test("Columns - No Chage column order - Up", async () => {
		await columns.columnsBtn.click();
	...
	test("Columns - Add column", async () => {
		await columns.columnsBtn.click();
		const item = await columns.getLeftItemByName(dataview.addItem);
		await columns.checkLeftItem(item);
	...
	test("Columns - Remove column", async () => {
		await columns.columnsBtn.click();
		const item = await columns.getLeftItemByName(dataview.removeItem);
		await columns.uncheckLeftItem(item);
	...
	test("Columns - Remove all columns", async () => {
		await columns.columnsBtn.click();
		await columns.uncheckAllItems();
	...
	test("Columns - Add all columns", async () => {
		await columns.columnsBtn.click();
		await columns.checkAllItems();
