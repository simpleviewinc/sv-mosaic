...
function LeftNavItemDesktop(props) {
    const { openName, onOpen, item } = props;
    ...
    const onNavClick = useCallback((event) => {
    ...
    const onOpenChildrenClick = function (e) {
        onOpen(item.name);
    };
    ...
    return (React.createElement(Fragment, null,
        React.createElement(LeftNavItem, { ...props, ref: aRef, attrs: attrs }),
        item.items && open &&
            React.createElement(LeftNavFlyout, { parent: item, anchorEl: aRef.current })));
}
