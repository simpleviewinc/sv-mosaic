...
* **events** - `any` optional - THIS HAS NEVER BEEN USED, CHECK WITH OWEN IF SHOULD BE DELETED FROM DOCS.
* **onSubmit** - `(...args: any) => any` optional - Function that will be executed when the user clicks on the save button of the form. THIS HAS NEVER BEEN USED SINCE WE USE registerOnSubmit, CHECK WITH OWEN IF SHOULD BE DELETED FROM DOCS.
...
### Dispatch
The dispatch function allows developers to update the state through the following actions:
...
We recommend encapsulating this function inside of a useMemo hook for better performance.
...
* **size** - `Size | string` optional - Size of the field. Could be either one of the predefined sizes (please see [Sizes section](#sizes-type)), or a custom size. This property can be internally overwritten if the field is placed inside of a row with other fields.
