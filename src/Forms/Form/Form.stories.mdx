...
```ts
import Form, { useForm, formActions, Sizes } from "sv-mosaic";
...
- inputSettings:
	...
	* **size** - `string` optional - Size of the field. Could be either one of the predefined sizes (see [Sizes section](#sizes-type)), or a custom size. This property can be internally overwritten if the field is placed inside of a row with other fields.
