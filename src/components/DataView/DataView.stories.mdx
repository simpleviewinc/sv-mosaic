...
* **onReorder** - `function` -  A callback function which will be executed when a row has ended being drag and drop. This callback will receive as an argument the rows updated after the reordening.
