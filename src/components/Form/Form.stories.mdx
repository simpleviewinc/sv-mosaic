...
- inputSettings:
	...
	* **options** - `object` optional -  Function used to get the full info (label, value) of all selected options. This prop only applies when getting options locally.
...
- inputSettings:
	* **options** - `array` of `{ label: string, value: string, selected?: boolean }` optional - Array of options to be rendered containing their corresponding label, value, and indicator of whether its selected or not.
	* **getOptions** - `() => Promise<MosaicLabelValue[]>` optional - Function to get a set of options. This prop only applies when getting options from a DB.
...
- inputSettings:
	...
	* **options** - `array` of `MosaicLabelValue` optional - Array of options to be displayed on the options.
	* **getOptions** - `() => Promise<MosaicLabelValue[]>` optional - Function to get a set of options. This prop only applies when getting options from a DB.
