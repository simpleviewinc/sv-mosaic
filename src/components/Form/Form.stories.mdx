...
## Methods
The methods object that contains a number of methods that can be used to manipulate the form state:
* **setFieldValue** - Updates the value of any single field by name and optionally validates the field and marks it as touched. Parameters are `{ name: string, value: any, validate?: boolean, touched?: boolean }`

## Other
* `handleSubmit` - Should be used to wrap a callback that will be invoked when the form is successfully submitted after validation. Takes an optional second callback to be invoked when validation fails. You should invoke the callback that is returned when you'd like submission to be triggered.

### Submit
You are responsible for triggering the submission process, either by providing the `Form` component with an `onSubmit` property, or by invoking the `onSubmit` function another way.
Your `onSubmit` function should be a callback that is wrapped with the `handleSubmit` method available on the form controller. This ensures your callback will only be called if form validation is successful.
`handleSubmit` accepts an optional second callback that will be invoked if validation fails.

```ts
const onSubmitSuccess = useCallback((data) => {
	alert("Submission was successful! The data submitted was: " + JSON.stringify(data));
}, []);
