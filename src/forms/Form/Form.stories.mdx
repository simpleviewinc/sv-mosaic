...
- inputSettings: `object`
	* **amountPerType** - `number | string` optional - When defined, limits the amount of address types. For example: If a dev adds “amountPerType: 2” then users will be able to add 2 physical, 2 billing, and 2 shipping addresses.
	* **amountShipping** - `number | string` optional - When defined, limits the amount of address types to "shipping" only. Takes precedence over amountPerType.
	* **amountPhysical** - `number | string` optional - When defined, limits the amount of address types to "physical" only. Takes precedence over amountPerType.
	* **amountBilling** - `number | string` optional - When defined, limits the amount of address types to "billing" only. Takes precedence over amountPerType.
